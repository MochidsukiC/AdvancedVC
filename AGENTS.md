...
- **定期的なデバッグ**: 巨大な実装を行う際、エージェントには出来ないデバッグを途中で行う必要がある場合は、手順を示した上でユーザーにデバッグを要求してください。
...
- **Codexの書き込み**: 書き込みはpowershellからの書き込みではなく、付属の書き込みツールを使用して下さい。powershellからの書き込みでは文字化けや、コメントアウトのずれなど大量の問題が発生します。
